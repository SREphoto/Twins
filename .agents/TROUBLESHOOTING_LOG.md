...
| **CHK-11** | **Internal Power Supply & Power Routing** | Floating electrical components with no power supply or wiring. | Modeled internal PSU (step-down transformer, bridge rectifier, smoothing filter capacitors, voltage regulators); color-coded 3D wires (Hot, Neutral, Ground bonded to chassis, DC rails). |
| **CHK-12** | **Sensor Data Gathering & Signal Routing** | Telemetry numbers magically appearing without sensors or wiring. | Modeled physical sensors (tachometer optical encoder / Hall sensor, NTC stator thermistor, microswitches) connected via ribbon cables to MCU; external data ports (RS-232, USB-B, RJ45) modeled if applicable. |
